<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use App\Models\Setting;

class SettingSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $settings = [
            [
                'key' => 'company_name',
                'value' => 'PT. Contoh Perusahaan',
                'type' => 'string',
                'description' => 'Nama perusahaan',
            ],
            [
                'key' => 'company_address',
                'value' => 'Jl. Contoh No. 123, Jakarta',
                'type' => 'string',
                'description' => 'Alamat perusahaan',
            ],
            [
                'key' => 'work_days_per_week',
                'value' => '6',
                'type' => 'integer',
                'description' => 'Jumlah hari kerja per minggu',
            ],
            [
                'key' => 'work_hours_per_day',
                'value' => '8',
                'type' => 'integer',
                'description' => 'Jumlah jam kerja per hari',
            ],
            [
                'key' => 'overtime_rate',
                'value' => '1.5',
                'type' => 'string',
                'description' => 'Rate lembur (kali dari gaji normal)',
            ],
            [
                'key' => 'auto_clock_out_time',
                'value' => '23:59:00',
                'type' => 'string',
                'description' => 'Waktu auto clock out jika lupa',
            ],
            [
                'key' => 'max_late_tolerance',
                'value' => '30',
                'type' => 'integer',
                'description' => 'Maksimal toleransi terlambat (menit)',
            ],
            [
                'key' => 'attendance_radius',
                'value' => '100',
                'type' => 'integer',
                'description' => 'Radius absensi default (meter)',
            ],
            [
                'key' => 'annual_leave_quota',
                'value' => '12',
                'type' => 'integer',
                'description' => 'Kuota cuti tahunan (hari)',
            ],
            [
                'key' => 'sick_leave_quota',
                'value' => '12',
                'type' => 'integer',
                'description' => 'Kuota cuti sakit (hari)',
            ],
            [
                'key' => 'photo_required_clock_in',
                'value' => '1',
                'type' => 'boolean',
                'description' => 'Wajib foto saat clock in',
            ],
            [
                'key' => 'photo_required_clock_out',
                'value' => '1',
                'type' => 'boolean',
                'description' => 'Wajib foto saat clock out',
            ],
            [
                'key' => 'location_required',
                'value' => '1',
                'type' => 'boolean',
                'description' => 'Wajib lokasi saat absen',
            ],
            [
                'key' => 'auto_absent_time',
                'value' => '10:00:00',
                'type' => 'string',
                'description' => 'Waktu otomatis mark absent jika belum clock in',
            ],
            [
                'key' => 'notification_late_threshold',
                'value' => '15',
                'type' => 'integer',
                'description' => 'Batas waktu untuk notifikasi terlambat (menit)',
            ],
            [
                'key' => 'payroll_cut_off_date',
                'value' => '25',
                'type' => 'integer',
                'description' => 'Tanggal cut off payroll setiap bulan',
            ],
            [
                'key' => 'weekend_overtime_rate',
                'value' => '2.0',
                'type' => 'string',
                'description' => 'Rate lembur akhir pekan',
            ],
            [
                'key' => 'holiday_overtime_rate',
                'value' => '3.0',
                'type' => 'string',
                'description' => 'Rate lembur hari libur',
            ],
            [
                'key' => 'max_photo_size',
                'value' => '2048',
                'type' => 'integer',
                'description' => 'Maksimal ukuran foto (KB)',
            ],
            [
                'key' => 'app_timezone',
                'value' => 'Asia/Jakarta',
                'type' => 'string',
                'description' => 'Timezone aplikasi',
            ],
            [
                'key' => 'grace_period_minutes',
                'value' => '5',
                'type' => 'integer',
                'description' => 'Grace period untuk clock in/out (menit)',
            ],
            [
                'key' => 'require_break_time',
                'value' => '1',
                'type' => 'boolean',
                'description' => 'Wajib clock out/in saat jam istirahat',
            ],
            [
                'key' => 'min_work_hours_full_pay',
                'value' => '7',
                'type' => 'integer',
                'description' => 'Minimum jam kerja untuk gaji penuh',
            ],
            [
                'key' => 'email_notifications',
                'value' => '1',
                'type' => 'boolean',
                'description' => 'Aktifkan notifikasi email',
            ],
            [
                'key' => 'sms_notifications',
                'value' => '0',
                'type' => 'boolean',
                'description' => 'Aktifkan notifikasi SMS',
            ],
            [
                'key' => 'backup_schedule',
                'value' => 'daily',
                'type' => 'string',
                'description' => 'Jadwal backup database (daily, weekly, monthly)',
            ],
            [
                'key' => 'report_email',
                'value' => 'hr@company.com',
                'type' => 'string',
                'description' => 'Email untuk laporan otomatis',
            ]
        ];

        foreach ($settings as $setting) {
            Setting::create($setting);
        }
    }
}
